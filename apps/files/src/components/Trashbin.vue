      ...
      <oc-table-group>
        <oc-table-row v-for="(item, index) in fileData" :key="index" class="file-row">
          ...
          <oc-table-cell class="uk-text-meta uk-text-nowrap">
            ...
            <oc-button
              :id="'files-trashbin-file-list-action-button-small-resolution-' + index"
              icon="more_vert"
              class="uk-hidden@m"
              :aria-label="$_ocTrashbin_getShowFileActionsLabel()"
              @click.stop
            />
            <oc-drop
              v-if="!ocDialogIsOpen"
              :toggle="'#files-trashbin-file-list-action-button-small-resolution-' + index"
              :options="{ offset: 0 }"
...
export default {
  ...
  methods: {
    ...
    $_ocTrashbin_getShowFileActionsLabel () {
      return this.$gettext('Show file actions')
    },
