      ...
      <li v-for="(link, index) in $_links" :key="'li-' + index">
        <oc-grid flex gutter="small">
          ...
          <div class="uk-width-auto uk-button-group">
            <button class="uk-button uk-button-default uk-position-relative" @click.stop="$_copyToClipboard(link, $event)" aria-label="copy link url to clipboard">
              <oc-icon name="copy_to_clipboard" class="uk-position-center" />
            </button>
            <oc-button :disabled="!!(formOpen || linkId)" icon="edit" @click="$_editLink(link)" ariaLabel="edit link"/>
            <oc-button :disabled="!!(formOpen || linkId)" icon="delete" @click="$_removeLink(link)" ariaLabel="remove link"/>
          </div>
        </oc-grid>
